...
function AuthButton({
  ...
  return (
    <button
      type="button"
      className={[
        'flex h-[86px] w-full items-center justify-center gap-4 rounded-[28px] text-[28px] font-medium tracking-[-0.03em] transition active:scale-[0.995]',
        dark
          ? 'bg-[linear-gradient(180deg,#1a1a1a,#050505)] text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.12)]'
          : 'border border-black/5 bg-white/80 text-black shadow-[0_1px_0_rgba(255,255,255,0.7)_inset]',
      ].join(' ')}
    >
      <span className={dark ? 'text-white' : 'text-black/80'}>{icon}</span>
      <span>{children}</span>
    </button>
  );
}
...
export function KivoAuthPanel() {
  return (
    <section className="relative rounded-[38px] border border-white/80 bg-white/34 p-6 shadow-[0_16px_60px_rgba(0,0,0,0.08),inset_0_1px_0_rgba(255,255,255,0.7)] backdrop-blur-[22px] sm:p-8">
      <div className="space-y-5">
        <AuthButton variant="dark" icon={<AppleIcon />}>
          Continue with Apple
        </AuthButton>
      ...
      <div className="pt-8 text-center text-[24px] font-normal tracking-[-0.02em] text-black/48">
