    ...
    >
      ...
      <Card className="premium-card bg-primary/10 border-primary/20 p-8">
        <div className="flex justify-between items-start gap-4">
          <div className="space-y-2">
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary">Monthly Reclaimed Protocol</p>
            <h3 className="text-5xl font-bold font-headline text-primary tracking-tighter">
              ${data.savingsEstimate || 0}
            </h3>
          </div>
      ...
      {data.beforeAfterComparison && (
        <div className="grid md:grid-cols-2 gap-4">
          <Card className="premium-card !p-5 bg-white/[0.02] border-white/5 space-y-3">
            ...
            <p className="text-xs font-medium italic opacity-60">"{data.beforeAfterComparison.currentSituation}"</p>
          </Card>
          <Card className="premium-card !p-5 bg-success/5 border-success/10 space-y-3">
            ...
            <p className="text-xs font-medium text-success/80">"{data.beforeAfterComparison.optimizedSituation}"</p>
          </Card>
        </div>
      )}
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-muted-foreground">Detected Anomalies ({(data.detectedItems || []).length})</p>
        </div>
        
        <div className="grid gap-3">
          {(data.detectedItems || []).map((item: any, idx: number) => (
            <Card key={idx} className="premium-card !p-0 overflow-hidden bg-white/[0.01] border-white/5 group">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[9px] font-bold text-success uppercase tracking-widest">Save ${item.estimatedSavings}/mo</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="px-5 pb-5 pt-0 space-y-4">
                <p className="text-xs text-muted-foreground leading-relaxed italic">"{item.summary}"</p>
