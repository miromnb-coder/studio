...
function IOSSwitch({ active, disabled }: { active: boolean; disabled?: boolean }) {
  return (
    <span className={`relative inline-flex h-[31px] w-[51px] shrink-0 items-center rounded-full transition-colors duration-200 ${active ? 'bg-[#0a84ff]' : 'bg-[#e5e5e7]'} ${disabled ? 'opacity-70' : ''}`}>
      <span className={`absolute top-[2px] h-[27px] w-[27px] rounded-full bg-white shadow-[0_2px_7px_rgba(0,0,0,0.22)] transition-transform duration-200 ${active ? 'translate-x-[22px]' : 'translate-x-[2px]'}`} />
    </span>
  );
}
...
function ConnectorSheetRow({ item, onAction }: { item: ConnectorSheetItem; onAction: (item: ConnectorSheetItem) => void }) {
  ...
  return (
    <li className="grid min-h-[76px] grid-cols-[36px_minmax(0,1fr)_96px] items-center gap-4 border-b border-black/[0.055] last:border-b-0">
      ...
      <div className="flex min-w-0 items-center gap-3">
        ...
        {item.beta ? (
          <span className="shrink-0 rounded-full border border-black/[0.12] bg-white/38 px-3 py-0.5 text-[15px] font-normal tracking-[-0.035em] text-[#8a8a8a]">
            Beta
          </span>
        ) : null}
      </div>
      ...
      <button
        ...
        className="justify-self-end text-[20px] font-normal tracking-[-0.04em] text-[#808080] active:opacity-60"
...
export function WorkspaceSheet({
    ...
    try {
      ...
      setConnectors((prev) => prev.map((item) => {
        if (item.id === 'gmail' && gmailData) {
          const updated: ConnectorRecord = { ...item, state: gmailData.connected === true ? 'connected' : 'not_connected', accountEmail: typeof gmailData.accountEmail === 'string' ? gmailData.accountEmail : null, lastSyncAt: typeof gmailData.lastSyncedAt === 'string' ? gmailData.lastSyncedAt : null };
          saveConnectorRecord('gmail', updated);
          return updated;
        }
        if (item.id === 'google-calendar' && calendarData) {
          const updated: ConnectorRecord = { ...item, state: calendarData.connected === true ? 'connected' : 'not_connected', accountEmail: typeof calendarData.accountEmail === 'string' ? calendarData.accountEmail : null, lastSyncAt: typeof calendarData.lastSyncAt === 'string' ? calendarData.lastSyncAt : null };
          saveConnectorRecord('google-calendar', updated);
          return updated;
        }
        if (item.id === 'browser' && browserData) {
          const updated: ConnectorRecord = { ...item, state: browserData.connected === true ? 'connected' : 'not_connected', accountEmail: null, lastSyncAt: null };
  ...
  const resolvedConnectors = useMemo(() => connectors.map((connector) => {
  ...
  const sheetItems = useMemo<ConnectorSheetItem[]>(() => {
    ...
    const primary = CONNECTOR_ORDER.map((id) => {
      ...
      return { id, connectorId: id, name: CONNECTOR_NAME_MAP[id] ?? connector.name, state: connector.state, canToggle: TOGGLE_AFTER_CONNECTED.has(id) } as ConnectorSheetItem;
    });
  ...
  const handleDragEnd = (_: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
    if (info.offset.y > 120 || info.velocity.y > 650) closeWithHaptic();
    else haptic.light();
  };
  ...
  const updateConnector = useCallback((id: ConnectorId, state: ConnectorState) => {
    ...
    setRuntimeState((prev) => ({ ...prev, [id]: { state, busy: state === 'connecting' || state === 'reconnecting' } }));
  }, []);

  const handleConnectorAction = useCallback((item: ConnectorSheetItem) => {
    haptic.selection();
  ...
  return (
    <AnimatePresence>
      {open ? (
        <>
          <motion.div className="fixed inset-0 z-40 bg-black/86 backdrop-blur-[1px]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={closeWithHaptic} />

          <motion.aside
            className="fixed inset-x-0 bottom-0 z-50 mx-auto h-[calc(100dvh-28px)] max-w-[560px] overflow-hidden rounded-t-[10px] bg-[#f6f6f4] text-[#303030] shadow-[0_-14px_30px_rgba(0,0,0,0.18)]"
            initial={{ y: '100%' }}
            animate={{ y: 0 }}
            exit={{ y: '100%' }}
          ...
          >
            <div className="flex h-full flex-col">
              <header className="relative h-[104px] shrink-0 px-8 pt-2">
                <button type="button" onPointerDown={(event) => dragControls.start(event)} className="mx-auto block h-7 w-28 touch-none pt-2" aria-label="Drag to dismiss">
                  <span className="mx-auto block h-1.5 w-[66px] rounded-full bg-black/18" />
                </button>

                <button type="button" onClick={closeWithHaptic} className="absolute left-8 top-[34px] grid h-9 w-9 place-items-center text-[#303030] active:opacity-55" aria-label="Close connectors">
                  <X className="h-8 w-8" strokeWidth={1.9} />
                </button>

                <h2 className="absolute left-1/2 top-[38px] -translate-x-1/2 text-center text-[23px] font-semibold tracking-[-0.04em] text-black">Connectors</h2>
              </header>

              <div className="min-h-0 flex-1 overflow-y-auto overscroll-y-contain px-7 pb-[max(32px,env(safe-area-inset-bottom))] [-webkit-overflow-scrolling:touch]">
                <section className="overflow-hidden rounded-[28px] bg-[#ededeb] px-7 py-2">
                  <button type="button" onClick={() => handleConnectorAction({ id: 'gmail', connectorId: 'gmail', name: 'Gmail', state: 'not_connected' })} className="grid min-h-[72px] w-full grid-cols-[36px_minmax(0,1fr)_28px] items-center gap-4 border-b border-black/[0.06] text-left active:opacity-60">
                    <Plus className="h-8 w-8 shrink-0" strokeWidth={1.9} />
                    <span className="truncate text-[22px] font-normal tracking-[-0.045em]">Add connectors</span>
                    <ChevronRight className="h-7 w-7 text-[#7b7b7b]" strokeWidth={2.2} />
                  </button>

                  <button type="button" onClick={() => handleConnectorAction({ id: 'gmail', connectorId: 'gmail', name: 'Gmail', state: 'connected' })} className="grid min-h-[72px] w-full grid-cols-[36px_minmax(0,1fr)_28px] items-center gap-4 text-left active:opacity-60">
                    <Settings2 className="h-8 w-8 shrink-0" strokeWidth={1.9} />
                    <span className="truncate text-[22px] font-normal tracking-[-0.045em]">Manage connectors</span>
                    <ChevronRight className="h-7 w-7 text-[#7b7b7b]" strokeWidth={2.2} />
                  </button>
                </section>

                <section className="mt-9 overflow-hidden rounded-[28px] bg-[#ededeb] px-7 py-2">
                  <ul>{sheetItems.map((item) => <ConnectorSheetRow key={item.id} item={item} onAction={handleConnectorAction} />)}</ul>
                </section>
              </div>
            </div>
          </motion.aside>
