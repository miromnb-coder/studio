    ...
    >
      <div className="pointer-events-auto rounded-[30px] border border-black/[0.012] bg-white px-[10px] pb-[9px] pt-[13px] shadow-[0_9px_22px_rgba(15,23,42,0.038)] transition-[box-shadow,transform] duration-200 ease-out [contain:layout_paint] focus-within:shadow-[0_12px_28px_rgba(15,23,42,0.052)]">
        <textarea
          ...
          className="block w-full resize-none border-0 bg-transparent px-[14px] py-0 text-[19px] font-normal leading-[1.32] tracking-[-0.026em] text-[#22252b] outline-none placeholder:text-[#bdbdbd]"
        ...
        <div className="mt-[14px] flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-[7px]">
            ...
            <button
              ...
              className={`kivo-pressable inline-flex h-[44px] w-[44px] shrink-0 items-center justify-center rounded-full transition-all duration-200 ease-out active:scale-[0.96] ${canSend && !isSending ? 'bg-[#1f2329] text-white shadow-[0_9px_18px_rgba(15,23,42,0.16)]' : 'bg-[#eeeeee] text-[#d7d7d7] shadow-none'}`}
            >
...
const DockCircleButton = memo(function DockCircleButton({ ariaLabel, onClick, icon, active = false }: DockCircleButtonProps) {
  return (
    <button
      ...
      className={`kivo-pressable inline-flex h-[44px] w-[44px] shrink-0 items-center justify-center rounded-full border transition-all duration-200 ease-out active:scale-[0.96] ${active ? 'border-black/[0.04] bg-[#f7f9ff] text-[#25272d] shadow-[0_4px_10px_rgba(15,23,42,0.03)]' : 'border-black/[0.03] bg-white text-[#25272d] shadow-[0_4px_10px_rgba(15,23,42,0.03)]'}`}
    >
      {icon}
    </button>
  );
