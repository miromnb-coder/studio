...
export const KivoComposerDock = memo(function KivoComposerDock({ value, onChange, onSend, onPlusClick, onQuickActionClick, onMicClick, canSend, isListening = false, isSending = false, placeholder = 'Assign a task or ask anything', keyboardOffset = 0, containerRef, desktopShiftX = 0 }: KivoComposerDockProps) {
  ...
  return (
    <div ref={containerRef} className="pointer-events-none fixed inset-x-0 z-40 mx-auto w-full max-w-none px-[22px] pb-2 transition-transform duration-[260ms] ease-[cubic-bezier(0.22,1,0.36,1)] will-change-transform" style={{ bottom: 'calc(env(safe-area-inset-bottom, 0px) + 8px)', transform: `translate3d(${desktopShiftX}px, ${keyboardOffset > 0 ? -keyboardOffset : 0}px, 0)` }}>
      <div className="pointer-events-auto rounded-[29px] border border-black/[0.018] bg-white px-[13px] pb-[10px] pt-[13px] shadow-[0_10px_24px_rgba(15,23,42,0.045)] transition-[box-shadow,transform] duration-200 ease-out [contain:layout_paint] focus-within:shadow-[0_13px_30px_rgba(15,23,42,0.06)]">
        <textarea ref={textareaRef} id="kivo-composer-textarea" value={value} onChange={(event) => onChange(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); send(); } }} rows={1} placeholder={placeholder} className="block w-full resize-none border-0 bg-transparent px-[15px] py-0 text-[19px] font-normal leading-[1.32] tracking-[-0.026em] text-[#22252b] outline-none placeholder:text-[#bebebe]" style={{ minHeight: `${MIN_TEXTAREA_HEIGHT}px`, maxHeight: `${MAX_TEXTAREA_HEIGHT}px`, overflowY: 'hidden' }} />
...
const DockCircleButton = memo(function DockCircleButton({ ariaLabel, onClick, icon, active = false }: DockCircleButtonProps) {
  return <button type="button" onClick={onClick} aria-label={ariaLabel} className={`kivo-pressable inline-flex h-[44px] w-[44px] shrink-0 items-center justify-center rounded-full border transition-all duration-200 ease-out active:scale-[0.96] ${active ? 'border-black/[0.045] bg-[#f7f9ff] text-[#25272d] shadow-[0_4px_10px_rgba(15,23,42,0.035)]' : 'border-black/[0.035] bg-white text-[#25272d] shadow-[0_4px_10px_rgba(15,23,42,0.035)]'}`}>{icon}</button>;
});
