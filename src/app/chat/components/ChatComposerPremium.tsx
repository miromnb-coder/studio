...
export function ChatComposerPremium(props: ChatComposerPremiumProps) {
  ...
  return (
    <div ref={composerRef} className="fixed bottom-[calc(82px+env(safe-area-inset-bottom))] left-1/2 z-30 w-full max-w-md -translate-x-1/2 px-4 pb-2">
      <AnimatePresence>
        {notice ? (
          <motion.p
            ...
            exit={{ opacity: 0, y: 6 }}
            className="mb-2 px-2 text-xs text-zinc-400"
          >
            {notice}
          </motion.p>
        ) : null}
      ...
      <AnimatePresence>
        {openPanel === 'add' ? (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 8 }}
            className="mb-2 rounded-2xl border border-white/10 bg-[#121212]/95 p-1.5 shadow-[0_16px_30px_rgba(0,0,0,0.42)] backdrop-blur-xl"
          >
      ...
      <div className="rounded-[26px] border border-white/10 bg-[#101010]/92 p-1.5 shadow-[0_20px_38px_rgba(0,0,0,0.45)] backdrop-blur-xl">
