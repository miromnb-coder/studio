  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-md bg-[#f8fafc] px-5 py-6 text-slate-900">
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="rounded-full bg-slate-100 p-2.5 text-slate-500"><Clock3 className="h-5 w-5" /></div>
      </div>
      ...
      <section className="rounded-[22px] border border-black/[0.04] bg-white p-6 shadow-[0_4px_14px_rgba(15,23,42,0.04)]">
        ...
        <p className="mt-2 text-[1rem] leading-relaxed text-slate-500">Review recent actions, runs, summaries, and saved outputs.</p>

        <div className="mt-6 space-y-3">
          {selectors.history.slice(0, 12).map((item, index) => (
            <div key={item.id} className="rounded-2xl bg-slate-50 px-4 py-4 text-sm leading-relaxed text-slate-700">
              {index + 1}. {item.title}
            </div>
          ))}
        </div>
