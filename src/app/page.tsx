...
export default function HomePage() {
  ...
  const insight = useMemo(() => 'You could save €32 this month by cancelling 2 overlapping subscriptions.', []);

  const sendToChat = (value: string) => {
    const trimmed = value.trim();
    if (!trimmed) return;

    const messages = readChatMessages();
    const updated = [...messages, makeMessage('user', trimmed, 'home')];
    ...
    <main className="mx-auto min-h-screen w-full max-w-md bg-[#f8f9fc] pb-28 shadow-[0_10px_32px_rgba(15,23,42,0.06)]">
      ...
      <section className="space-y-6 px-6 py-7">
        ...
        <article className="overflow-hidden rounded-[20px] border border-black/[0.04] bg-white shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
          <div className="space-y-5 p-6">
            ...
            <textarea
              ...
              rows={3}
              placeholder="Ask Operator to analyze, summarize, or automate…"
        ...
        <article className="rounded-[20px] border border-emerald-100 bg-emerald-50/70 p-5 shadow-[0_2px_8px_rgba(0,0,0,0.03)]">
          ...
          <p className="mt-2 text-lg font-medium leading-snug text-emerald-900">{insight}</p>
          <button type="button" onClick={() => sendToChat('Show me the subscription cancellation steps to save €32 this month.')} className="mt-4 rounded-full bg-white px-4 py-2 text-sm font-semibold text-emerald-700">Review savings plan</button>
        </article>
        ...
        <article className="rounded-[20px] border border-black/[0.04] bg-white p-6 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
          ...
          <div className="space-y-5">
            {recentActivity.map(({ title, time, context, icon: Icon, prompt: activityPrompt }) => (
              <button key={title} onClick={() => sendToChat(activityPrompt)} className="flex w-full items-start gap-3 text-left">
        ...
        <section className="grid grid-cols-5 gap-3">
          ...
          <div className="col-span-2 space-y-3">
            ...
            <article className="rounded-[20px] border border-black/[0.04] bg-white p-5 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
              ...
              <p className="text-base text-slate-400">2 active leaks</p>
            </article>
          </div>
        </section>
      </section>
